...
export const AdminWehshop = () => {
  ...
  const fetchExistingImage = async () => {
    try {
      const response = await getWebshopImage();
      if (response) {
        setExistingImage(response.image);
        console.log("image", response.image);

        setFormData({
          title: response.title || "webshop",
      ...
      <div className="flex flex-col gap-14 max-w-[848px] mx-auto">
        <div className="flex flex-col gap-3.5">
          <h2 className="text-[#111727] font-medium">
            Upload webshop Wallpaper
          </h2>
          <p className="text-[#6C7275] text-sm">
            You can upload an image for the webshop wallpaper. By saving, the
